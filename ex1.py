...
if name == 'name':
if not (value and value[0].isupper() and value.replace("
", "").isalpha()):
...
with open(subjects_file, newline='', encoding='utf-8')
as csvfile:
...
raise FileNotFoundError(f"Файл {subjects_file} не
найден")
...
# Добавляем результат теста по предмету с проверкой
корректности
if subject not in self.subjects:
print(f"Предмет {subject} не найден")
return
...
100):
print("Результат теста должен быть целым числом от 0 до
100")
return
...
# Создаем экземпляр студента, загружаем предметы из файла
subjects.csv
student = Student("Иван Иванов", "subjects.csv")
